...
module ErrorHandling
  def check_and_print_error(connection = @connection, status = Bolt::Connection.status(connection), error_text = nil)
    ...
    if error_code == Bolt::Error::BOLT_SERVER_FAILURE
      ...
      string_buffer[4095] = 0 if Bolt::Values.bolt_value_to_string(Bolt::Connection.failure(connection),
      ...
      puts("#{error_text || 'Bolt failure'} (code: #{error_code.to_s(16)}, text: #{Bolt::Error.string(error_code)}, " \
           "context: #{error_ctx})")
    end
    error_code
  end
