    ...
    module Async
      module Pool
        class ConnectionPoolImpl
          ...
          def open?(address)
            Util::LockUtil.execute_with_lock(@address_to_pool_lock.read_lock, -> () {@address_to_pool.keys.include?(address)})
          end
          ...
          def process_acquisition_error(pool, server_address, error)
            ...
            if !cause.nil?
              if cause.is_a?(java.util.concurrent.TimeoutException)
                ...
                raise Neo4j::Driver::Exceptions::ClientException, "Unable to acquire connection from the pool within configured maximum time of #{settings.connection_acquisition_timeout}ms"
              elsif pool.closed?
                ...
                raise Neo4j::Driver::Exceptions::ServiceUnavailableException, "Connection pool for server #{server_address} is closed while acquiring a connection.", cause
              else
                # some unknown error happened during connection acquisition, propagate it
                raise java.util.concurrent.CompletionException, cause
              end
            end
          end
          ...
          def assert_not_closed(address = nil, channel = nil, pool = nil)
            if @closed.get
              ...
              raise java.lang.IllegalStateException, Spi::ConnectionPool::CONNECTION_POOL_CLOSED_ERROR_MESSAGE
            end
          end
          ...
          def get_or_create_pool(address)
            existing_pool = Util::LockUtil.execute_with_lock(@address_to_pool_lock.read_lock, -> () { @address_to_pool[:address] })
          ...
          def shutdown_event_loop_group(poll_close_error)
            ...
            # This timeout is needed for `close_pool_in_background` to finish background job, especially for races between `acquire` and `close`.
          ...
          def close_all_pools
            java.util.concurrent.CompletableFuture.all_of(
                @address_to_pool.entry_set.stream.map do |key, value|
                  address = key
                  pool = value
