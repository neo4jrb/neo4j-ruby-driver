        ...
        class NettyChannelTracker
          ...
          def channel_created(channel, creating_event = nil)
            if creating_event.nil?
              raise Neo4j::Driver::Exceptions::IllegalStateException, 'Untraceable channel created.'
            else
          ...
          def prepare_to_close_channels
            @all_channels.each do |channel|
              ...
              begin
                ...
                # only logging it
                @log.debug("Failed to prepare to close Channel #{channel} due to error #{e.message}. It is safe to ignore this error as the channel will be closed despite if it is successfully prepared to close or not.")
              end
            end
          end
          ...
          def decrement_in_use(channel)
            ...
            unless @address_to_in_use_channel_count.keys.include?(address)
              raise Neo4j::Driver::Exceptions::IllegalStateException.new("No count exists for address '#{address}' in the 'in use' count")
            end

            count = @address_to_in_use_channel_count[:address]
            @address_to_in_use_channel_count[:address] = count - 1
          end
          ...
          def decrement_idle(channel)
            ...
            unless @address_to_idle_channel_count.keys.include?(address)
              raise Neo4j::Driver::Exceptions::IllegalStateException, "No count exists for address '#{address}' in the 'idle' count"
            end

            count = @address_to_idle_channel_count[:address]
            @address_to_idle_channel_count[:address] = count - 1
          end
