...
module Neo4j::Driver::Internal
  class SecuritySetting
    ...
    def assert_security_settings_not_user_configured(uri_scheme)
      if customized?
        raise Neo4j::Driver::Exceptions::ClientException.new("Scheme #{uri_scheme} is not configurable with manual encryption and trust settings")
      end
    end

    def customized?
      self.encrypted != DEFAULT_ENCRYPTED || self.trust_strategy != DEFAULT_TRUST_STRATEGY
    end
    ...
    def create_security_plan_impl(encrypted, trust_strategy)
      ...
      when Config::TrustStrategy::TRUST_ALL_CERTIFICATES
        return org.neo4j.driver.internal.security.SecurityPlanImpl.forAllCertificates(hostname_verification_enabled, revocation_strategy)
      else
