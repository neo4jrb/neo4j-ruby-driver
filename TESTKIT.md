...
10. **`tests/stub` work** — bootstrap done (baseline + CI). Walk-down sequence:
    ...
    3. **Routing** — slice 1 done (skeleton, +5 routing tests passing). Slice 2: TTL-based and failure-driven refresh. Slice 3: leader-switch + transient retry. Slice 4: per-database routing + ROUTE `db` field. Slice 5: routing context + multiple seed routers. Slice 6: bookmark/lifecycle edge cases.
